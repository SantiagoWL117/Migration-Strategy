#!/usr/bin/env python3
"""
Extract ALL 164 active V1 restaurant data using proper MySQL dump parsing
Based on proven MVP extraction script
"""
...
# Load ALL restaurant mappings
print("Loading V1 to V3 restaurant mapping...")
mapping = {}
...
target_v1_ids = list(mapping.keys())
print(f"Loaded {len(target_v1_ids)} restaurant mappings\n")
...
for insert_idx, insert_pos in enumerate(insert_positions):
    ...
    # Check each record for target restaurant IDs
    for record in records:
        ...
        if restaurant_id not in target_v1_ids:
            continue
        
        found_count += 1
        if found_count % 10 == 0:
            print(f"  Found {found_count}/{len(target_v1_ids)} restaurants...")
...
print(f"Found {len(results)} out of {len(target_v1_ids)} target restaurants")
print(f"{'='*60}\n")

# Save results
csv_path = 'all_restaurants_extracted_data.csv'
...
for blob_type, blobs in blob_data.items():
    if blobs:
        json_path = f'all_restaurants_blob_{blob_type}.json'
        with open(json_path, 'w', encoding='utf-8') as f:
            json.dump(blobs, f, indent=2, ensure_ascii=False)
        print(f"Saved {blob_type} BLOBs to: {json_path} ({len(blobs)} entries)")
