...
3. `audit_log_2025_10` - 489 MB
4. `audit_log_2025_11` - 100 MB
5. `dishes` - 38 MB
6. `users` - 33 MB
...
#### Edge Functions (4 functions)
- `check-legacy-account` - Check if user needs migration
...
**⚠️ RECOMMENDATION**: These can likely be removed after migration is complete.
...
|--------|--------|------|---------------|----------------|----------|
| **Audit & Logging** | 9 | **600 MB** | 4 | 1 | Archive/Cleanup |
...
| **Legacy Migration** | 0 | **0 MB** | 0 | 4 | **DELETE** |
| **TOTAL** | **110** | **~1.1 GB** | **178** | **38** | - |
...
1. **Archive Old Audit Logs** (Save ~580 MB)
   - `audit_log_2025_10` (489 MB)
   - `audit_log_2025_11` (100 MB)
   - Action: Archive to cold storage, keep last 30 days
...
1. **Confirm Entity Groupings** - Review with team
2. **Prioritize Cleanup Tasks** - Which entities to tackle first?
3. **Create Detailed Plans** - One entity at a time
4. **Execute Cleanup** - Measure impact

**Awaiting your direction on which entity to start with!**
