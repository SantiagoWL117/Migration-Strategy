-- FIX #6: Filter out empty courses from output
-- 
-- Problem: Courses with no active dishes are included in output with "dishes": []
-- This creates unnecessary data and confuses the frontend.
--
-- Solution: Add EXISTS clause to only include courses that have at least one dish
-- matching the current filter criteria (active/inactive based on p_active_items_only)

CREATE OR REPLACE FUNCTION menuca_v3.get_restaurant_menu(
  p_restaurant_id bigint, 
  p_language_code text DEFAULT 'en'::text, 
  p_active_items_only boolean DEFAULT true
)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'active_items_only', p_active_items_only,
    'courses', COALESCE(
      jsonb_agg(
        jsonb_build_object(
          'id', c.id,
                    ...
                    jsonb_build_object(
                      ...
                      'modifiers', COALESCE((
                            ...
                            'prices', COALESCE((
                              SELECT jsonb_agg(
                                jsonb_build_object(
                                  'id', mp.id,
                                  'size_variant', CASE WHEN v_use_french 
                                    THEN COALESCE(msv.name_fr, msv.name_en, mp.size_variant) 
                                    ELSE COALESCE(msv.name_en, msv.name_fr, mp.size_variant) 
                                  END,
                              ...
                              LEFT JOIN menuca_v3.modifier_size_variants msv ON msv.id = mp.modifier_size_variant_id
                        ...
                        WHERE m.modifier_group_id = mg.id 
                          ...
                          AND (NOT p_active_items_only OR m.is_active = true)
                      ), '[]'::jsonb)
                    ...
                    jsonb_build_object(
                      ...
                      'sections', COALESCE((
                                        ...
                                        'prices', COALESCE((
                                          SELECT jsonb_agg(
                                            jsonb_build_object(
                                              'id', cmp.id,
                                              'size_variant', CASE WHEN v_use_french 
                                                THEN COALESCE(msv.name_fr, msv.name_en, cmp.size_variant) 
                                                ELSE COALESCE(msv.name_en, msv.name_fr, cmp.size_variant) 
                                              END,
                        ...
                        WHERE cgs.combo_group_id = cg.id 
                          AND (NOT p_active_items_only OR cgs.is_active = true)
                      ), '[]'::jsonb)
                    ) ORDER BY cg.id
                  )
  ...
  WHERE c.restaurant_id = p_restaurant_id
    ...
    -- FIX #6: Only include courses that have at least one matching dish
    AND EXISTS (
