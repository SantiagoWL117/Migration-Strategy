-- SQL Migration: Populate French translations for generic dishes
-- Source: gap_generic_dishes.csv
-- Date: 2026-01-12

BEGIN;

-- 1. Create a temporary staging table
CREATE TEMPORARY TABLE generic_dishes_staging (
    name_en TEXT,
    name_fr TEXT
);

-- 2. Import the CSV data
\copy generic_dishes_staging (name_en, name_fr) FROM 'gap_generic_dishes.csv' WITH (FORMAT CSV, HEADER TRUE, ENCODING 'UTF8');

-- 3. Log loaded translations
SELECT 'Loaded translations:' AS status, COUNT(*) AS count FROM generic_dishes_staging;

-- 4. Clean up empty/null entries
DELETE FROM generic_dishes_staging
WHERE name_en IS NULL OR TRIM(name_en) = ''
   OR name_fr IS NULL OR TRIM(name_fr) = '';

SELECT 'After cleanup:' AS status, COUNT(*) AS count FROM generic_dishes_staging;

-- 5. Update dishes table
UPDATE menuca_v3.dishes d
SET name_fr = s.name_fr,
    updated_at = NOW()
FROM generic_dishes_staging s
...
DROP TABLE generic_dishes_staging;
