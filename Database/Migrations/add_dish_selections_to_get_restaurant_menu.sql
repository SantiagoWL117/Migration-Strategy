-- Add combo_group_dish_selections to get_restaurant_menu function
-- This allows the frontend to render combo dish selections properly
                ...
                'combo_groups', COALESCE((
                  SELECT jsonb_agg(
                    jsonb_build_object(
                      ...
                      'dish_selections', COALESCE((
                        SELECT jsonb_agg(
                          jsonb_build_object(
                            'id', cgds.id,
                            'dish_id', cgds.dish_id,
                            'size', cgds.size,
                            'dish_display_name', cgds.dish_display_name,
                            'course_id', cgds.course_id,
                            'dish_name', CASE WHEN v_use_french 
                              THEN COALESCE(sel_d.name_fr, sel_d.name_en) 
                              ELSE COALESCE(sel_d.name_en, sel_d.name_fr) 
                            END,
                            'dish_is_active', sel_d.is_active
                          ) ORDER BY cgds.id
                        )
                        FROM menuca_v3.combo_group_dish_selections cgds
                        JOIN menuca_v3.dishes sel_d ON sel_d.id = cgds.dish_id AND sel_d.deleted_at IS NULL
...
-- Verify update
SELECT 'get_restaurant_menu updated with dish_selections support' as status;
