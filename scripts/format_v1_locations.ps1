...
# Input CSV path
$csvPath = Join-Path $PSScriptRoot '..\Database\Restaurant Management Entity\restaurants_locations\CSV\menuca_v1_restaurants_locations.csv'
...
function Normalize-Address([string]$a) {
  ...
  $decoded = [System.Web.HttpUtility]::HtmlDecode($a)
  # Replace non-breaking spaces with regular spaces
  $decoded = $decoded -replace "\u00A0", ' '
  # Collapse excessive whitespace
  $collapsed = ($decoded -replace '\s{2,}', ' ').Trim()
  # Normalize to NFC to preserve composed diacritics like é, ô
  return $collapsed.Normalize([Text.NormalizationForm]::FormC)
}
...
foreach ($row in $rows) {
  if ($row.PSObject.Properties.Name -contains 'address') { $row.address = Normalize-Address $row.address }
