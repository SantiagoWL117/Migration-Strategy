#!/usr/bin/env python3
"""
Batch scraper for dish prices and modifiers.
Processes existing dishes and adds price and modifier data.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('batch_scrape_prices_modifiers.log'),
...
def get_dishes_to_process(db: DatabaseManager, limit: int = None) -> List[Dict]:
    """Get dishes that have source_id (menu_entry_id) from database."""
    query = f"""
        SELECT 
            d.id as dish_id,
            d.restaurant_id,
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Batch Prices & Modifiers Scraper")
    logger.info("=" * 60)
    ...
    # For testing, start with a small limit (e.g., 10 dishes)
    # Remove limit for full run
    dishes = get_dishes_to_process(db, limit=None)  # Change to None for full run
    ...
    logger.info("\n" + "=" * 60)
    logger.info("BATCH SCRAPING SUMMARY - PRICES & MODIFIERS")
    logger.info("=" * 60)
    logger.info(f"Duration: {duration}")
    logger.info(f"Total dishes in database: {len(dishes)}")
    logger.info(f"\nProgress Status:")
    logger.info(f"  Already completed: {len(completed_dish_ids)}")
    logger.info(f"  Previously failed: {len(failed_dish_ids)}")
    logger.info(f"  Previously skipped: {len(skipped_dish_ids)}")
    ...
    logger.info(f"  Log: batch_scrape_prices_modifiers.log")
    logger.info("=" * 60)
