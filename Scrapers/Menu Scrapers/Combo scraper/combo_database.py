...
class ComboDatabase:
    ...
    def get_dish_by_name(self, restaurant_id: int, dish_name: str) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            self.cursor.execute(fuzzy_query, (clean_name, restaurant_id, clean_name))
            result = self.cursor.fetchone()
            return dict(result) if result else None
            ...
            """
            self.conn.rollback()
            self.cursor.execute(ilike_query, (restaurant_id, f"%{clean_name}%"))
            result = self.cursor.fetchone()
            return dict(result) if result else None
        ...
        try:
            ...
            if existing:
                ...
                insert_query = f"""
                    ...
                    RETURNING id
                """
                self.cursor.execute(insert_query, (dish_id, combo_group_id, is_active))
                result = self.cursor.fetchone()
    ...
    def insert_combo_modifier_group(self, combo_group_section_id: int, name: str,
        ...
        try:
            ...
            check_query = f"""
                ...
                LIMIT 1
            """
            self.cursor.execute(check_query, (combo_group_section_id, source_id))
            existing = self.cursor.fetchone()
        ...
        try:
            ...
            if existing:
                ...
                update_query = f"""
                    ...
                    RETURNING id
                """
                self.cursor.execute(update_query, (display_order, existing['id']))
                result = self.cursor.fetchone()
            else:
                # Insert new
                insert_query = f"""
