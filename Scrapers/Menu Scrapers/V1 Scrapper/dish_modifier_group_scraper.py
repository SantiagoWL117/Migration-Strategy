"""
Dish to Modifier Group Linker Scraper

Links English dishes to their modifier groups and extracts modifier group details.
...
# =============================================================================
# CATEGORY ID TO DETAILS FIELD MAPPING
# =============================================================================

# Maps the category div ID to the HTML field IDs for extracting details
CATEGORY_DETAILS_MAP = {
    ...
    'br': {  # Bread
        ...
        'min_id': None,  # No min/max/free for bread
        'max_id': None,
        'free_id': None,
    ...
    'cm': {  # Cooking Method
        ...
        'header_id': 'cmHeader',
        'min_id': None,  # No min/max/free for cooking method
        'max_id': None,
        'free_id': None,
...
def get_restaurants_with_dishes(db: DatabaseConnection, logger) -> List[Dict]:
    """Get all English restaurants with V1 IDs and their dish counts.
    Excludes French restaurants (handled by french_dish_modifier_group_scraper.py).
    """
    # French restaurant V3 IDs to exclude
    french_v3_ids = [727, 1009, 1011, 644, 1012, 1013, 1014, 1015, 1016, 1017]
    ...
    query = f"""
        ...
        ORDER BY r.name
    """
    results = db.execute_with_retry(query, french_v3_ids, fetch=True)
    restaurants = []
    for row in results:
        restaurants.append({
            'v3_id': row[0],
    ...
    logger.info(f"Found {len(restaurants)} restaurants with {sum(r['dish_count'] for r in restaurants)} dishes")
    return restaurants
...
def get_processed_dishes(db: DatabaseConnection, logger) -> set:
    """Get set of dish IDs that already have modifier_group_details."""
    query = """
        SELECT DISTINCT dish_id FROM menuca_v3.modifier_group_details
    """
    results = db.execute_with_retry(query, fetch=True)
    processed = set(row[0] for row in results) if results else set()
    logger.info(f"Found {len(processed)} dishes with existing modifier_group_details")
    return processed
...
async def scrape_dish_modifier_groups(page: Page, db: DatabaseConnection,
    ...
    url = f"{CRM_BASE_URL}/?p=restaurants&display=editRestaurant&restaurant={restaurant['v1_id']}&load=editDish&showLang=en&menuEntry={dish['source_id']}"
...
async def run_dish_modifier_scraper():
    ...
    logger.info("=" * 60)
    logger.info("DISH TO MODIFIER GROUP LINKER SCRAPER")
    logger.info("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        for i, restaurant in enumerate(restaurants):
            ...
            try:
                ...
                total_dishes_processed += dishes_processed
                total_dishes_skipped += dishes_skipped
                total_links_created += links
                total_details_created += details
                
                logger.info(f"  Result: {dishes_processed} processed, {dishes_skipped} skipped, {links} links, {details} details")
...
if __name__ == "__main__":
    asyncio.run(run_dish_modifier_scraper())
