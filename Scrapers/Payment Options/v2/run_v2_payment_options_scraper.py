"""
V2 Payment Options Scraper - Runner Script

Scrapes payment options from V2 CRM for all V2 restaurants and updates
the menuca_v3.restaurant_payment_options table.

Usage:
    python run_v2_payment_options_scraper.py           # Scrape all V2 restaurants
    python run_v2_payment_options_scraper.py --test    # Test with first 5 restaurants
    python run_v2_payment_options_scraper.py --dry-run # Show what would be updated without making changes
"""
...
from v2_payment_options_scraper import (
    setup_logging,
    DatabaseConnection,
    login_to_crm,
...
async def main():
    """Main entry point for the V2 payment options scraper."""
    
    # Parse arguments
    parser = argparse.ArgumentParser(description='Scrape V2 restaurant payment options')
    ...
    # Setup logging
    logger = setup_logging('v2_payment_options_scraper')
    
    logger.info("=" * 60)
    logger.info("V2 Payment Options Scraper")
    logger.info("=" * 60)
    ...
    try:
        # Get V2 restaurants
        restaurants = get_v2_restaurants(db, logger)
        ...
        if args.restaurant_id:
            ...
            if not restaurants:
                logger.error(f"Restaurant with V3 ID {args.restaurant_id} not found or is not a V2 restaurant")
                return
        ...
        async with async_playwright() as p:
            ...
            for i, restaurant in enumerate(restaurants, 1):
                ...
                try:
                    ...
                    scraped = await scrape_restaurant_payment_options(page, v2_id, logger)
