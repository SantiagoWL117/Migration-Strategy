...
DROP FUNCTION IF EXISTS menuca_v3.get_my_admin_info() CASCADE;
DROP FUNCTION IF EXISTS menuca_v3.assign_restaurants_to_admin(BIGINT, BIGINT[], TEXT) CASCADE;
DROP FUNCTION IF EXISTS menuca_v3.create_admin_user_request(TEXT, TEXT, TEXT, TEXT) CASCADE;

-- =====================================================
-- Helper Function: Get current admin user info
-- =====================================================
CREATE OR REPLACE FUNCTION menuca_v3.get_my_admin_info()
RETURNS TABLE(
  admin_id BIGINT,
  email TEXT,
  first_name TEXT,
...
BEGIN
  ...
  SELECT id, status INTO v_caller_admin_id, v_caller_status
  ...
  SELECT email, status INTO v_target_admin_email, v_target_admin_status
  FROM menuca_v3.admin_users
  ...
  ELSIF p_action = 'remove' THEN
    ...
    WHERE admin_user_id = p_admin_user_id
    AND restaurant_id = ANY(v_valid_restaurants);
    GET DIAGNOSTICS v_affected = ROW_COUNT;
...
COMMENT ON FUNCTION menuca_v3.assign_restaurants_to_admin(BIGINT, BIGINT[], TEXT) IS
...
-- =====================================================
CREATE OR REPLACE FUNCTION menuca_v3.create_admin_user_request(
  p_email TEXT,
  p_first_name TEXT,
  p_last_name TEXT,
...
BEGIN
  ...
  RETURN QUERY SELECT
    ...
    p_email,
    'pending'::TEXT,
    format('Admin user created with id %s. NEXT STEPS:
1. Create auth account in Supabase Dashboard (Authentication → Users → Add User)
2. Copy the auth user UUID
3. Update admin_users.auth_user_id with the UUID
4. Update admin_users.status to ''active''',
      v_new_admin_id
    );
END;
$$;

COMMENT ON FUNCTION menuca_v3.create_admin_user_request(TEXT, TEXT, TEXT, TEXT) IS
...
-- =====================================================
GRANT EXECUTE ON FUNCTION menuca_v3.get_my_admin_info() TO authenticated;
GRANT EXECUTE ON FUNCTION menuca_v3.assign_restaurants_to_admin(BIGINT, BIGINT[], TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION menuca_v3.create_admin_user_request(TEXT, TEXT, TEXT, TEXT) TO authenticated;
