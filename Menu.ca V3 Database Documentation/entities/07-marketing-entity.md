...
- [📊 Tables](#-tables) — `promotional_deals`, `promotional_coupons`, `coupon_usage_log`, `promotion_campaigns`, `promotion_codes`, `promotion_targets`, `promotion_tiers`, `promotion_redemptions`, `promotion_templates`, `marketing_tags`, `restaurant_tag_associations`
- [👁️ Views](#️-views) — `active_promotional_coupons`
...
#### `restaurant_tag_associations` (1,625 records)
**Purpose:** Join table linking restaurants to marketing tags
...
| `created_at` | timestamptz | NO | Assignment date |

> **Note:** All translation tables have been deleted. Bilingual support is via direct columns (`name_en`/`name_fr`, `description_en`/`description_fr`).
...
| Function | Parameters | Returns | Description |
|----------|------------|---------|-------------|
| `add_tag_to_restaurant` | restaurant_id, tag_id | void | Assign a marketing tag to a restaurant |
| `create_restaurant_tag` | name_en, name_fr, slug | record | Create a new marketing tag |
| `get_restaurants_by_tag` | tag_slug | TABLE | Get restaurants with a specific tag |
...
### `marketing_tags` Table Indexes (2)
...
### `restaurant_tag_associations` Table Indexes (3)

| Index Name | Columns | Type | Condition |
|------------|---------|------|-----------|
| `restaurant_tag_associations_pkey` | `id` | PRIMARY KEY | - |
| `restaurant_tag_associations_restaurant_id_tag_id_key` | `restaurant_id, tag_id` | UNIQUE | - |
| `idx_restaurant_tag_assoc_tag` | `tag_id` | BTREE | - |
...
### `restaurant_tag_associations` Table Policies

| Policy Name | Operation | Roles | Description |
|-------------|-----------|-------|-------------|
| `public_view_tag_associations` | SELECT | public | Public can view associations |
| `tag_assoc_service_role_all` | ALL | service_role | Full access |
...
**Marketing Hub (2026-01-26):** Added 6 features — bilingual translations (direct `_en`/`_fr` columns), coupon validation at checkout, deal analytics dashboard, per-customer usage limits, tiered discounts, and item targeting. All translation tables dropped in favor of direct columns.

**Columns dropped from `promotional_deals` (12):** `type`, `description`, `order_count_required`, `required_items`, `free_item_count`, `image_url`, `is_customizable`, `is_split_deal`, `shows_on_thankyou`, `sends_in_email`, `email_body_html`, `language_code`.

**Columns dropped from `promotional_coupons` (8):** `name`, `applies_to_items`, `item_count`, `coupon_scope`, `email_text`, `is_used`, `language_code`, `v1_coupon_id`, `v2_coupon_id`, `source_table`.

**Tables dropped (3):** `promotional_deals_translations`, `promotional_coupons_translations`, `marketing_tags_translations`.

**Functions dropped (4):** `get_coupon_with_translation`, `translate_marketing_tag`, duplicate `validate_coupon` (5-param), duplicate `clone_deal` (integer).

**Data fixes:** Standardized `deal_type` (10 legacy → 5 standard). Migrated `included_items` to V3 IDs. Deleted 100 expired coupons. Translated all tags/coupons to French. Auto-assigned 1,625 tag associations.
...
|---|-------|----------|--------|-------|
| 1 | ~~`restaurant_tag_associations` duplicate indexes~~ | ✅ | Resolved | Dropped 5 duplicate indexes (2026-02-17) — kept pkey, UNIQUE composite, and 1 tag_id index |
| 2 | ~~`idx_marketing_tags_slug` redundant~~ | ✅ | Resolved | Dropped (2026-02-17) — UNIQUE constraint covers lookups |
| 3 | Campaign system tables are all empty (0 rows) | ℹ️ Info | Expected | New system not yet launched |
| 4 | `coupon_usage_log` has 0 records | ℹ️ Info | Expected | No coupons redeemed yet |
| 5 | Legacy migration columns on `promotional_deals` (`v1_deal_id`, `v2_deal_id`, `v1_meal_number`, `v1_position`, `v1_is_global`, `v2_deal_id`, `deal_type_legacy`, `included_items_legacy`) | 🟡 Low | Open | Migration complete — candidates for future cleanup |
...
| RLS Policies | 15 |
| Triggers | 3 |
| Promotional Deals | 56 |
| Promotional Coupons | 352 |
| Marketing Tags | 36 |
| Tag Associations | 1,625 |
| Promotion Templates | 8 |
