#!/usr/bin/env python3
"""
V2 Delivery & Schedule Scraper - Main Entry Point

Scrapes delivery time, takeout time, schedules, and service settings
from the V2 CRM (aggregator-admin.menu.ca) for all restaurants with 
legacy_v2_id (and no legacy_v1_id).

Usage:
    python main.py                    # Scrape all V2 restaurants
    python main.py --test             # Test with first 3 restaurants
    python main.py --restaurant 1678  # Scrape specific V2 ID
...
from V2.scraper import V2DeliveryScheduleScraper
from V2.config import OUTPUT_DIR, LOG_DIR
...
def scrape_restaurants(restaurants: List[Dict[str, Any]], 
    ...
    Args:
        restaurants: List of dicts with 'id', 'name', 'legacy_v2_id'
    ...
    with V2DeliveryScheduleScraper(headless=headless) as scraper:
        if not scraper.login():
            logging.error("Failed to login to V2 CRM. Aborting.")
            return results
        ...
        for idx, restaurant in enumerate(restaurants, 1):
            ...
            result = scraper.scrape_restaurant(v3_id, v2_id, name)
...
def main():
    parser = argparse.ArgumentParser(description='V2 Delivery & Schedule Scraper')
    parser.add_argument('--test', action='store_true', 
                        help='Test mode: scrape only first 3 restaurants')
    parser.add_argument('--restaurant', type=int, 
                        help='Scrape specific V2 restaurant ID')
    ...
    log_file = LOG_DIR / f'v2_scraper_{timestamp}.log'
    setup_logging(str(log_file))
    
    logging.info("=" * 60)
    logging.info("V2 Delivery & Schedule Scraper")
    logging.info("=" * 60)
    ...
    with DatabaseManager() as db:
        if args.restaurant:
            ...
            restaurants = [r for r in db.get_v2_restaurants() 
                          if r['legacy_v2_id'] == args.restaurant]
            if not restaurants:
                logging.error(f"Restaurant with V2 ID {args.restaurant} not found")
                return
        else:
            restaurants = db.get_v2_restaurants()
            if args.test:
    ...
    output_file = OUTPUT_DIR / f'v2_scraped_data_{timestamp}.json'
    save_results(results, output_file)
    
    # Also save to standard filename for easy access
    latest_file = OUTPUT_DIR / 'v2_scraped_data.json'
