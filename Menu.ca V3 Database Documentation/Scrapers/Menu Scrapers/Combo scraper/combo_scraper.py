...
class ComboScraper:
        ...
        try:
            ...
            if 'p=login' in current_url or self.page.query_selector('input[name="username"]'):
                ...
                if 'error' in page_content.lower() or 'invalid' in page_content.lower() or 'wrong' in page_content.lower():
                    ...
                    logger.error(
                        "Still on login page - form may not have submitted correctly")
                # Save the page for debugging
                with open('debug_login_failed.html', 'w', encoding='utf-8') as f:
                    f.write(page_content)
                logger.info(
                    "Saved failed login page to debug_login_failed.html")
                return False
            ...
            if self.page.query_selector('ul#active') or self.page.query_selector('.restaurantList'):
                ...
                logger.warning(
                    f"Login status unclear - page saved to debug_after_login.html")
    ...
    def navigate_to_combo_groups(self, v1_id: int) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    "Combo group elements detected - page fully loaded")
            except:
                # No combo groups found after waiting - this is valid (restaurant may not have any)
                logger.info(
                    "No combo group elements detected after waiting - page may be empty or have no groups")
    ...
    def get_combo_group_links(self) -> List[Dict[str, Any]]:
        ...
        if 'editGroupJS' in html_content:
            ...
            logger.info(
                "'editGroupJS' NOT found in page HTML - checking for login form...")
            if 'Username' in html_content or 'Password' in html_content:
                logger.warning(
                    "Login form detected! Session may have been lost.")
        ...
        for selector in selectors:
            ...
            if links:
                logger.debug(
                    f"Selector '{selector}' found {len(links)} elements")
                break
            else:
            ...
            for a_tag in soup.find_all('a', onclick=True):
                ...
                if 'editGroupJS' in onclick:
                    ...
                    if match:
                        ...
                        logger.debug(
                            f"Found combo group (via BS4): {name} (source_id={source_id})")
        else:
            for link in links:
                ...
                if match:
                    ...
                    logger.debug(
                        f"Found combo group: {name} (source_id={source_id})")
    ...
    def _parse_section(self, soup: BeautifulSoup, config: Dict) -> Optional[Dict[str, Any]]:
        ...
        if order_input:
            try:
                section_data['display_order'] = int(
                    order_input.get('value', 0) or 0)
            except ValueError:
                pass
        ...
        if config.get('min_input'):
            ...
            if min_input:
                try:
                    section_data['min_selection'] = int(
                        min_input.get('value', 0) or 0)
                except ValueError:
                    pass
        ...
        if config.get('max_input'):
            ...
            if max_input:
                try:
                    section_data['max_selection'] = int(
                        max_input.get('value', 0) or 0)
                except ValueError:
                    pass
        ...
        if config.get('free_input'):
            ...
            if free_input:
                try:
                    section_data['free_items'] = int(
                        free_input.get('value', 0) or 0)
                except ValueError:
                    pass
        ...
        if ul:
            section_data['modifier_groups'] = self._parse_modifier_groups(
                ul, config['section_type'])
    ...
    def _parse_modifier_groups(self, ul: BeautifulSoup, section_type: str) -> List[Dict[str, Any]]:
        ...
        for li in ul.find_all('li', recursive=False):
            ...
            if not is_selected:
                logger.debug(
                    f"Skipping unchecked modifier group: source_id={source_id}")
                continue
            ...
            type_code = radio.get('class', [''])[
                0].upper() if radio.get('class') else 'RADIO'

            # Parse modifiers from the nested ul
            modifiers = []
            nested_ul = li.find(
                'ul', {'class': type_code.lower() if type_code else None})
            if not nested_ul:
                # Try finding any ul
    ...
    def scrape_combo_groups(self, restaurant_id: int, v1_id: int) -> Dict[str, int]:
        ...
        for cg_info in combo_group_links:
            logger.info(
                f"Processing combo group: {cg_info['name']} (source_id={cg_info['source_id']})")
            ...
            if not combo_group_id:
                logger.error(
                    f"Failed to insert combo group: {cg_info['name']}")
                continue
            ...
            for section in cg_data['sections']:
                ...
                if not section_id:
                    logger.error(
                        f"Failed to insert section: {section['section_type']}")
                    continue
                ...
                for mg in section['modifier_groups']:
                    ...
                    if not mg_id:
                        logger.error(
                            f"Failed to insert modifier group: {mg['name']}")
                        continue
                    ...
                    for modifier in mg['modifiers']:
                        ...
                        if not modifier_id:
                            logger.error(
                                f"Failed to insert modifier: {modifier['name']}")
                            continue
    ...
    def parse_combo_dish_page(self) -> Dict[str, Any]:
        ...
        drinks_header = soup.find(
            'p', string=re.compile(r'Drinks', re.IGNORECASE))
        if drinks_header:
            # Parse drinks modifiers similar to other sections
            drinks_div = drinks_header.find_next_sibling('div')
            if drinks_div:
                data['drinks_modifiers'] = self._parse_drinks_section(
                    drinks_div)

        return data

    def _parse_drinks_section(self, div: BeautifulSoup) -> Dict[str, Any]:
        """Parse drinks modifier section.

        Returns a dict with:
        - group_name: Name of the drinks modifier group
        - source_id: V1 source ID of the group
        - modifiers: List of individual drink modifiers with name and price
        """
        result = {
            'group_name': 'Drinks',
            'source_id': None,
        ...
        # Get source_id from the radio value
        source_id = checked_radio.get('value')
        if source_id:
            try:
                result['source_id'] = int(source_id)
            except ValueError:
                pass

        # Get group name from label
        label = div.find('label', {'for': checked_radio.get('id')})
        if label:
            result['group_name'] = label.get_text().strip() or 'Drinks'
    ...
    def scrape_combo_dishes(self, restaurant_id: int, v1_id: int) -> Dict[str, int]:
        ...
        if not combo_dishes:
            logger.info(
                f"No combo dishes found for restaurant {restaurant_id}")
            return stats

        # Process each combo dish
        for dish_info in combo_dishes:
            logger.info(
                f"Processing combo dish: {dish_info['name']} (combo_id={dish_info['combo_id']})")
            ...
            dish = self.db.get_dish_by_name(
                restaurant_id, dish_data['name'] or dish_info['name'])
            if not dish:
                logger.warning(
                    f"Dish not found in database: {dish_data['name']}")
                continue
            ...
            for cg in dish_data['assigned_combo_groups']:
                combo_group = self.db.get_combo_group_by_source_id(
                    restaurant_id, cg['source_id'])
                if combo_group:
                    result = self.db.insert_dish_combo_group(
                        dish_id, combo_group['id'])
                    if result:
                        stats['combo_links_created'] += 1
                else:
                    logger.warning(
                        f"Combo group not found: source_id={cg['source_id']}")
    ...
    def _insert_drinks_modifiers(self, restaurant_id: int, dish_id: int,
                                 drinks_data: Dict[str, Any]) -> int:
        """Insert drinks modifiers into the database.

        Args:
            restaurant_id: V3 restaurant ID
            dish_id: V3 dish ID
            drinks_data: Dict with group_name, source_id, and modifiers list

        Returns:
            Number of drink modifiers successfully inserted
        """
        inserted_count = 0

        group_name = drinks_data.get('group_name', 'Drinks')
        modifiers = drinks_data.get('modifiers', [])

        if not modifiers:
            return 0

        # Insert modifier group for this dish
        modifier_group_id = self.db.insert_modifier_group(
            dish_id=dish_id,
            name=group_name,
            is_required=False,
            min_selections=0,
            # Allow selecting any number of drinks
            max_selections=len(modifiers),
            free_items=0,
            display_order=100  # Put drinks at the end
        )

        if not modifier_group_id:
            logger.warning(
                f"Failed to create drinks modifier group for dish {dish_id}")
            return 0

        logger.debug(
            f"Created drinks modifier group: {group_name} (id={modifier_group_id})")

        # Insert each drink modifier and its price
        for modifier in modifiers:
            drink_name = modifier.get('name', '')
            drink_price = modifier.get('price', 0.0)
            ...
            if not modifier_id:
                logger.warning(
                    f"Failed to insert drink modifier: {drink_name}")
                continue

            # Insert the price (using 'Standard' size variant)
            ...
            if price_id:
                inserted_count += 1
                logger.debug(
                    f"Inserted drink: {drink_name} @ ${drink_price:.2f}")
            else:
                logger.warning(
                    f"Failed to insert price for drink modifier: {drink_name}")
