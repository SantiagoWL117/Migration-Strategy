"""
V2 Admin Contacts Scraper Runner

Orchestrates the scraping of admin contacts from V2 CRM and stores them
in the menuca_v3 database.

Usage:
    python run_v2_admin_scraper.py                # Run all 20 restaurants
    python run_v2_admin_scraper.py --test 1637   # Test with single V2 ID
    python run_v2_admin_scraper.py --dry-run     # Preview without database changes

Features:
- Hardcoded list of 20 V2 restaurants
- Edge case handling: Duplicates, missing emails, test contacts
...
from v2_admin_contacts_scraper import (
    setup_logging,
    DatabaseConnection,
    login_to_v2_crm,
...
# =============================================================================
# V2 Restaurants to Scrape (hardcoded from instructions)
# =============================================================================
V2_RESTAURANTS = [
    # (v3_id, name, v2_id)
...
async def process_restaurant_contacts(
    ...
    try:
        # Step 1: Scrape contacts from V2 CRM
        all_contacts = await scrape_restaurant_contacts(page, v2_id, logger)
        ...
        for contact in unique_contacts:
            ...
            try:
                ...
                if existing_admin:
                    ...
                    # Create new admin
                    # First create Supabase Auth user (or get existing)
                    auth_user_id = await create_supabase_auth_user(email, logger)
                    ...
                    # Note: auth_user_id may be 'EXISTS' if user exists but we don't have their ID
        ...
        error_msg = f"Error processing restaurant {name} (V2: {v2_id}): {e}"
...
async def run_scraper(
    ...
    Args:
        restaurants: List of (v3_id, name, v2_id) tuples. Defaults to V2_RESTAURANTS.
        dry_run: If True, don't make database changes.
        test_v2_id: If set, only process this V2 restaurant ID.
    """
    logger = setup_logging("v2_admin_contacts")
    logger.info("=" * 60)
    logger.info("V2 ADMIN CONTACTS SCRAPER")
    logger.info("=" * 60)
    ...
    if test_v2_id:
        # Test mode - find restaurant by V2 ID
        restaurant = None
        for v3_id, name, v2_id in V2_RESTAURANTS:
            if v2_id == test_v2_id:
                restaurant = {'v3_id': v3_id, 'name': name, 'v2_id': v2_id}
                break
        
        if not restaurant:
            logger.error(f"No restaurant found with V2 ID {test_v2_id}")
            db.close()
            return
        
        restaurants = [restaurant]
        logger.info(f"TEST MODE: Processing single restaurant V2 ID {test_v2_id}")
    elif restaurants is None:
        restaurants = [
            {'v3_id': v3_id, 'name': name, 'v2_id': v2_id}
            for v3_id, name, v2_id in V2_RESTAURANTS
        ]
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            for idx, restaurant in enumerate(restaurants, 1):
                logger.info("-" * 40)
                logger.info(f"[{idx}/{total}] {restaurant['name']} (V3: {restaurant['v3_id']}, V2: {restaurant['v2_id']})")
                ...
                try:
                    ...
                    totals['test_contacts_skipped'] += stats.get('test_contacts_skipped', 0)
...
def main():
    """CLI entry point."""
    parser = argparse.ArgumentParser(description='V2 Admin Contacts Scraper')
    parser.add_argument('--test', type=int, metavar='V2_ID',
                        help='Test with a single V2 restaurant ID')
