...
#### Pachino Pizza 3515 Albion Road South (Restaurant ID: 974)
**Status:** ⚠️⚠️⚠️ CRITICAL ISSUE - All 98 dishes in "Uncategorized", proper courses exist but empty
**Date:** 2025-11-03
**Address:** 3515 Albion Road South ✅ (matches verified list)
**Assignee:** Brian (B)
**Menu link:** NEEDED (to verify course structure and ensure all dishes are present)

**Step 1: Restaurant Status**
```sql
SELECT id, name, status FROM menuca_v3.restaurants WHERE name ILIKE '%Pachino Pizza%';
```
- Restaurant ID: 974
- Name: Pachino Pizza
- Status: active ✅ (matches verified billing list)
...
**Step 2: Check Courses**
```sql
SELECT COUNT(*) FROM menuca_v3.courses WHERE restaurant_id = 974;
```
- Courses defined: 13 ✅ (but all have 0 dishes except Uncategorized)
...
SELECT
    ...
    COUNT(CASE WHEN course_id IS NOT NULL THEN 1 END) as has_course_id_count
FROM menuca_v3.dishes
WHERE restaurant_id = 974 AND deleted_at IS NULL;
```
- Total dishes: 98 ✅
- Dishes with NULL course_id: 0 (0%) ✅
- Dishes with course_id: 98 (100%) ✅
- **CRITICAL ISSUE:** All 98 dishes assigned to "Uncategorized" course

**Step 4: Check Course Structure**
```sql
SELECT c.id, c.name, COUNT(d.id) as dish_count FROM menuca_v3.courses c LEFT JOIN menuca_v3.dishes d ON c.id = d.course_id AND d.deleted_at IS NULL WHERE c.restaurant_id = 974 GROUP BY c.id, c.name ORDER BY c.display_order;
```
- Courses defined: 13 ✅
- Course distribution:
  - Deals: 0 dishes
  - Kebab Combos: 0 dishes
  - Pizza: 0 dishes
  - Appetizers: 0 dishes
  - Salads: 0 dishes
  - Poutine: 0 dishes
  - Platters: 0 dishes
  - Kabab Platters: 0 dishes
  - Kabab Sandwiches on Charcoal: 0 dishes
  - Subs: 0 dishes
  - Middle Eastern Pies: 0 dishes
  - Drinks: 0 dishes
  - Uncategorized: 98 dishes ⚠️⚠️⚠️
- **CRITICAL ISSUE:** All 12 proper courses exist but are EMPTY. All 98 dishes incorrectly assigned to "Uncategorized" course. Need to reassign all dishes to appropriate courses.
...
SELECT 
    ...
    COUNT(DISTINCT dm.dish_id) as dishes_with_modifiers
FROM menuca_v3.dish_modifiers dm
WHERE dm.restaurant_id = 974 AND dm.deleted_at IS NULL;
```
- Total modifiers: 74 ✅
- Dishes with modifiers: 43 ✅
...
LEFT JOIN menuca_v3.courses c ON d.course_id = c.id
LEFT JOIN menuca_v3.dish_modifiers dm ON d.id = dm.dish_id AND dm.deleted_at IS NULL
WHERE d.restaurant_id = 974 AND d.deleted_at IS NULL
GROUP BY d.id, d.name, c.name
HAVING COUNT(DISTINCT dm.id) > 0
ORDER BY modifier_count DESC;
```
...
LEFT JOIN menuca_v3.dishes d ON dm.dish_id = d.id
LEFT JOIN menuca_v3.ingredient_groups ig ON dm.ingredient_group_id = ig.id
WHERE dm.restaurant_id = 974 AND dm.deleted_at IS NULL AND d.deleted_at IS NULL
GROUP BY dm.dish_id, d.name, dm.ingredient_group_id, ig.name
ORDER BY dm.dish_id, dm.ingredient_group_id;
```
